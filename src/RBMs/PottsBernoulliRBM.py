...
def init_parameters(num_visibles : int,
    ...
    eps = 1e-4
    init_std = 1e-4
    num_states = np.max(dataset) + 1
    dataset_oh = torch.eye(num_states)[dataset].to(device)
    frequencies = dataset_oh.mean(0)
    frequencies = torch.clamp(frequencies, min=eps, max=(1. - eps))

    vbias = torch.log(frequencies) - 1. / num_states * torch.sum(torch.log(frequencies), 0)
    hbias = torch.zeros(num_hiddens, device=device)
    weight_matrix = torch.randn(size=(num_visibles, num_states, num_hiddens), device=device) * init_std
...
def init_parallel_chains(num_chains : int,
    ...
    Args:
        ...
        num_visibles (int): Number of visible units.
        num_hiddens (int): Number of hidden units.
        num_states (int): Number of Potts states.
        device (torch.device): Device.

    Returns:
        Tuple[Tensor, Tensor]: Initial visible and hidden units.
    """
    v = torch.randint(0, num_states, size=(num_chains, num_visibles), device=device).type(torch.int64)
    h = torch.randint(0, num_states, size=(num_chains, num_hiddens), device=device).type(torch.float32)
    return (v, h)
...
def compute_gradient(data : Tuple[Tensor, Tensor, Tensor],
                     parallel_chains : Tuple[Tensor, Tensor],
                     num_states : int) -> Tuple[Tensor, Tensor, Tensor]:
    ...
    v_data_one_hot = one_hot(v_data, num_classes=num_states).float()
    v_gen_one_hot = one_hot(v_gen, num_classes=num_states).float()
    
    # Averages over data and generated samples
    v_data_mean = (v_data_one_hot * w_data.unsqueeze(-1)).sum(0) / w_data.sum()
    ...
    # Gradient
    grad_weight_matrix = torch.tensordot((v_data_centered * w_data.unsqueeze(-1)), h_data_centered, dims=[[0], [0]]) / w_data.sum() \
        - torch.tensordot(v_gen_centered, h_gen_centered, dims=[[0], [0]]) / num_chains
    grad_vbias = v_data_mean - v_gen_mean - (grad_weight_matrix @ h_data_mean)
    grad_hbias = h_data_mean - h_gen_mean - torch.tensordot(v_data_mean, grad_weight_matrix, dims=[[0, 1], [0, 1]])
...
def update_parameters(data : Tuple[Tensor, Tensor, Tensor],
    ...
    num_states = params[2].shape[1]
    grad = compute_gradient(data=data, parallel_chains=parallel_chains, num_states=num_states)
    ...
    # Zero-sum gauge
    weight_matrix_new -= weight_matrix_new.mean(1, keepdim=True)
